...
class ExperienceType extends CI_Controller {
    public function __construct()
    {
        parent::__construct();
    }
    public function create_experience_type(){
        try{
            $data=array();
            $insert=array();
...
//			$request = json_decode($postdata);
            ...
            if(isset($request->experiencetypename) && preg_match("/^[a-zA-Z0-9+ ]{3,20}$/",$request->experiencetypename)){
                ...
                $status=false;
                echo $request->experiencetypename;
            }
            ...
            if($status){
                if(isset($request->txtid) && is_numeric($request->txtid)){
                    if($request->txtid>0){
                        ...
                        $res=$this->Model_Db->update(43,$insert,"id",$request->txtid);
                        if($res!=false){
                        ...
                        $where="type='$request->experiencetypename'";
                        $duplicate_check=$this->Model_Db->select(43,null,$where);
                        if($duplicate_check!=false){
                            ...
                            $res=$this->Model_Db->insert(43,$insert);
                            if($res!=false){
    ...
    public function load_experience_type(){
        try{
            $data=array();
            $where="isactive=true";
            $res=$this->Model_Db->select(43,null,$where);
    ...
    public function report_experience_type(){
        try{
            $data=array();
            $request= json_decode(json_encode($_POST), false);
//			$postdata = file_get_contents("php://input");
