...
//			$request = json_decode($postdata);
            $status=true;
            if(isset($request->skillname) && preg_match("/^[a-zA-Z ]{3,20}$/",$request->skillname)){
                $insert[0]['skill']=$request->skillname;
            }else{
                $status=false;
                echo $request->skillname;
            }
            ...
            if($status){
                if(isset($request->txtid) && is_numeric($request->txtid)){
                    if($request->txtid>0){
                        ...
                        $res=$this->Model_Db->update(41,$insert,"id",$request->txtid);
                        if($res!=false){
                        ...
                        $where="skill='$request->skillname'";
                        $duplicate_check=$this->Model_Db->select(41,null,$where);
                        if($duplicate_check!=false){
                            ...
                            $res=$this->Model_Db->insert(41,$insert);
                            if($res!=false){
    ...
    public function load_skill(){
        try{
            $data=array();
            $where="isactive=true";
            $res=$this->Model_Db->select(41,null,$where);
            $data[]="<option value=''>Select</option>";
            if($res!=false){
                foreach ($res as $r){
                    $data[]="<option value='$r->id'>$r->skill</option>";
                }
            }
            echo json_encode($data);
        }catch (Exception $e){
